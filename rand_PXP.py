...
def rand_dir_prod_states(number:int, length:int, device=tc.device('cuda:0'), dtype=tc.complex128)->tc.Tensor:
    number = int(number)
    shape = [number, 2]
...
def Z2_states(number:int, length:int, device=tc.device('cuda:0'), dtype=tc.complex64):
