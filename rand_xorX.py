...
def xorX_mul_states_evl(states, para=None):
    ...
    eye[0, 0] = 1+0.j
    eye[1, 1] = 1+0.j
    hamilt_1 = para['lambda'] * (tc.kron(eye, tc.kron(sigma_x, eye)) - tc.kron(sigma_z, tc.kron(sigma_x, sigma_z)))
...
def gen_select(gen_type:str):
    if gen_type == 'product' or gen_type == 'd':
        ...
        gen = first_n_xorX_states
    return gen

def label_generate(data, para):
    label = xorX_mul_states_evl(data, para)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_type', type=str, default='xorX')
    ...
    mkdir(path)
    para_evol = dict(para)
    para_evol['delta'] = args.delta
    para_evol['lambda'] = args.lambda_
    para_evol['J'] = args.J
    ...
    E = E.reshape(shape_)
    evol_mat = xorX_mul_states_evl(E, para_evol).reshape(E.shape[0], -1)
