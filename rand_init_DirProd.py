...
if __name__ == 'main':
    ...
    parser.add_argument('--folder', type=str, default='rand_dir_prod_init/')
    ...
    trainset = rand_dir_prod_states(args.train_num, length, device=para['device'])
    print(trainset.dtype)
    train_lbs = Heisenberg_mul_states_evl(trainset, para)
    testset = rand_dir_prod_states(0.2 * args.train_num, length, device=para['device'])
